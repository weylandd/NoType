...
4. **No audio on disk — ever.** PCM lives in the in-memory ring buffer; encoded m4a blobs are transient. History stores text only — as an ordered sequence of response segments, each carrying the chunk positions it covered and either the model's raw text or a gap; a lost chunk's *position* is persisted, never its audio. **One in-memory carve-out:** a broken row's failed-chunk blobs are held in `RetainedAudioStore` for the lifetime of the process so the user can re-send them; nothing is serialized and process exit ends it. → `NoType/History/CLAUDE.md` invariant 4.
5. **No injection mid-recording, and at most one injection per session — sometimes none.** Text is pasted at most once, after the final chunk's response arrives; partial transcripts are internal state only. **Zero times is a normal outcome, not a failure:** the paste is withheld when the frontmost process at paste time is not the process the user was in when they *stopped* recording, because a transcript arriving after a long transcription would otherwise edit a document the user never aimed at. The row is still written and a notice offers to copy it. → `NoType/Recording/CLAUDE.md` "Destination guard", `NoType/UI/CLAUDE.md` invariant 8.
