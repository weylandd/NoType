...
2. `xcodebuild archive` (Release configuration) **unsigned** (`CODE_SIGNING_ALLOWED=NO`), then `ditto` the `.app` out of the archive and code-sign it inside-out by hand with the Developer ID identity + hardened runtime — Sparkle's nested XPC services / `Updater.app` / `Autoupdate` / framework first, then the app with `NoType/NoType.entitlements` and the stable `identifier "app.notype"` designated requirement (`signing/NoType.xcrequirements`). No `-exportArchive`: the `keychain-access-groups` entitlement (#70) makes `xcodebuild` demand a provisioning profile even under Manual signing, so the script sidesteps its signing machinery entirely (see the comment in `scripts/release.sh`).
