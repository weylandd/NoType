...
- R20. A transcription request gives up after a short silence rather than the current thirty seconds. The threshold is set from measured healthy-response times, not from preference.
...
- AE11. A stalled chunk costs seconds, not a minute
  - **Covers R20, R21, R22.**
  - **Given** a transport that returns nothing at all.
  - **When** a dictation's chunk is sent and its single retry also returns nothing.
  - **Then** the session ends and the hotkey is usable again in well under half the time today's budget takes, and the row carries its audio for a re-send.
...
- The exact silence threshold R20 sets. U1 step 1 owns it: measure the two payload shapes the sample lacks, then set it. A measurement landing near 30 s is a blocker, not a value.
- Which timer produces the 20-second stalls. U1 step 2 owns the diagnosis; if it is not the inactivity timer, Phase A addresses only part of the observed problem.
...
- KTD1. **Cut the inactivity budget (`timeoutIntervalForRequest`); leave the whole-transfer budget at 30 s.** The inactivity timer is what fires on a stalled transport — but it also bounds Gemini's own think time, because the response is not streamed and the whole window from the last uploaded byte to the first response byte is idle. This cut is therefore a direct cap on how long any chunk is allowed to take, and the transfer budget cannot rescue a request the idle timer already killed: it is an additional ceiling, never a fallback. (Inherits KD7.) Governs R20.
- KTD2. **Measure before fixing the constant.** The healthy tail is 6.05 s at 119 KB, but the sample contains neither a full multi-chunk batch nor a 180 s force-cut chunk — the two shapes the retired 30 s ceiling was sized for. Measure both end-to-end, set the budget above the measured maximum with margin, and record the arithmetic beside the constant. **If the measurement lands near 30 s, stop and raise it as a blocker**: the wait would then have to come from a different mechanism rather than a flat cut. Governs R20.
...
- **Goal.** Cut the inactivity budget from 30 s to a measured value, retry over a fresh connection, and expose the budget and the retry ladder to tests for the first time.
- **Requirements.** R20, R21, R22, R28. KTD1, KTD2, KTD3, KTD13.
- **Dependencies.** None.
- **Files.** `NoType/Gemini/GeminiClient.swift`; `NoTypeTests/GeminiRetryPolicyTests.swift` (new); `NoTypeTests/GeminiClientOfflineShortCircuitTests.swift`.
- **Approach.**
  1. **Measure first (KTD2).** Capture end-to-end wall-clock for a full multi-chunk batch and for a force-cut-sized chunk — neither is in the sample. Raise a blocker if the maximum lands near 30 s.
  2. **Diagnose the 20 s class.** Reproduce one stall with task metrics captured and record which phase consumes it. If it is not the inactivity timer, the cut addresses only part of the observed problem; say so in the constant's comment rather than leaving it implied.
  3. Hoist the inactivity budget to a `nonisolated static let` whose doc-comment carries the measured maximum from step 1, the chosen margin, and the step-2 answer.
  4. Leave `timeoutIntervalForResource` at 30 s and record that it is an additional ceiling, not a fallback (KTD1).
...
- **Execution note.** Steps 1 and 2 are measurements, not code. Do them before the constant is fixed — the plan deliberately does not name a value.
...
- **Test scenarios.**
  ...
  - The inactivity budget is strictly less than the resource budget, and **strictly greater than the maximum measured in step 1** — not merely greater than the healthy tail.
  - Covers AE11. Two exhausted attempts at the new budget sum to well under half the retired 30 s ceiling.
  - A status-0 retry does not reuse the connection that failed.
- **Verification.** The suite passes; the budget and the retry ladder each fail a mutation of their value; the request-path source scan still anchors.
...
- **Approach.**
  ...
  4. **Leave the pause-ladder prose alone.** It cites `timeoutIntervalForResource`, which KTD1 preserves at 30 s, so `PauseDetector.swift`, `Recording/CLAUDE.md` invariant 4, and the adaptive-pause solution doc stay correct and are out of this unit.
- **Test scenarios.**
...
- **Requirements.** R16.
- **Dependencies.** U8.
- **Files.** `NoType/Dictionary/CLAUDE.md`; `NoType/History/CLAUDE.md`; `NoType/History/RetryMerge.swift`; `NoType/UI/CLAUDE.md`; `docs/solutions/architecture-patterns/partial-recovery-with-markers-2026-05-16.md`; `docs/solutions/architecture-patterns/hallucination-length-gate-2026-05-20.md`; `docs/solutions/design-patterns/local-chunk-concatenation-2026-05-15.md`; `docs/TECHDEBT.md`; `docs/solutions/documentation-gaps/` (new entry).
