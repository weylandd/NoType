...
- **Decision: Repetitive-pack collapse drops the pack — no summary line.** Per user direction, we don't render "(× 24 thumbnails)" replacement strings: that would just trade structured noise for prose noise and re-spend tokens. **Why:** the disambiguation contract is "spelling reference for proper nouns the speaker mentioned" (per system prompt); image filenames the user didn't say aren't part of that contract.
