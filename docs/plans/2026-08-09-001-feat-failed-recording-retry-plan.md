...
- R9. A broken row carrying no recovered text renders the design's placeholder bars in place of the transcript. They animate only while a retry is in flight — on a row that is merely broken, or whose audio is gone, they sit still so the row does not read as loading.
