...
- **Falsifier — the tripwire the superseded plan did not have.** Four outcomes are possible after the hand-off round, and only three of them are this plan being right:
  ...
  | **crash** | **AE4 — the interceptor missed a throw** (**amended 2026-07-30**; this cell read *"THIS PLAN IS WRONG"*) — see below | **R3** — negative result; the log names the next target |

  **Crash plus a silent interceptor — amended 2026-07-30, and the amendment inverts the conclusion.** As originally written this quadrant read: armed-and-silent ⇒ *no ObjC exception is being raised in that process* ⇒ the plan's central premise is wrong ⇒ stop enumerating throwers and reopen diagnosis from the `.ips`. **That inference is unsound, and acting on it would close a plan that is still standing.** A field crash report from tester A on `0.1.13-rc2` (build 17, macOS 26.2 25C56, 2026-07-30) carries the `HIServices SOME_OTHER_THREAD_SWALLOWED_AT_LEAST_ONE_EXCEPTION` marker thread — an ObjC exception demonstrably **was** swallowed, on a build that already carried the interceptor, and the interceptor is not displaced (see Sources). A silent log therefore no longer licenses "nothing threw". Handle the quadrant in this order:

  1. **Check the R4a armed line first — unchanged, and still the first move.** It separates *"the interceptor never installed"* (round **void**, re-run before concluding anything) from *"it installed"*.
  2. **Armed and silent means the interceptor missed a throw that independently happened** — not that no throw happened. Two live explanations, and the next round chases these rather than a new suspect: **(a)** the throw **bypassed `objc_setExceptionPreprocessor`** — `objc_exception_rethrow` (a `@throw;` re-raised from inside a `@catch` does not run the preprocessor) or a non-ObjC C++ throw; **(b)** the unified log's **size-bounded store rolled the record over** before it was read — check the `--last` window against how long ago the crash happened, and whether the copy had been running since before that window opened.
  3. **The plan's central premise is corroborated twice, independently** — the local swallow-path probe and now the rc2 field report — **not weakened.** Do not treat KTD5's containment shapes as inapplicable on the strength of a silent log.
  4. **The operational warning is unchanged, and is the reason this table exists.** Do **not** respond by adding another suspect thrower — that is the failure mode that cost the superseded plan three months. Do **not** respond by removing the reader at the crash site either — that is what failed three times before it (KD1).
...
- AE4. The crash survives and the interceptor is silent — the instrument missed it (**amended 2026-07-30**)
  - **Covers R3, R4a.**
  - **Superseded reading, kept because it was acted on as normative:** this example used to conclude *"no ObjC exception was raised in that process … this plan's premise is wrong … reopen diagnosis from the `.ips` rather than shipping a fifth suspect."* The first inference does not follow, and the rc2 field report (Sources) contradicts it directly: an exception **was** swallowed on an interceptor-carrying build. Only the conclusion changed — the four branches and the do-not-add-a-suspect instruction did not.
  ...
  - **Then** the interceptor **missed a throw**, and the next questions are which bypass it took — `objc_exception_rethrow` (a `@throw;` re-raise from inside a `@catch` skips the preprocessor) or a non-ObjC C++ throw — and whether the record was simply **rolled out of the size-bounded log store** before it was read (check the query window against the crash's age). Record the result on issue #82 as a missed instrument, not as a premise-level negative.
  - **And** the premise itself stands: a swallowed ObjC exception orphaning the executor identity is corroborated by the local probe **and** by the rc2 marker thread. Do not add a fifth suspect thrower, and do not remove the reader at the crash site (KD1) — both are the failure modes this example exists to block.
...
- KTD7. **Keep the version string at `0.1.13-rc1`; bump `CFBundleVersion` per hand-off build.** `v0.1.13-rc1` was never tagged or released — it exists only as a commit on `main` — so the string is free to reuse. Bumping the integer build (15 → 16 → …) keeps rounds distinguishable in the issue log without minting a version for a stage that turned out not to be the fix. **Promotion to `0.1.13` is NOT gated on a tester** (KD8, R22) — it follows the project's ordinary release cadence, on the merits of what is on `main`. The one thing that stays gated is KD5's narrow rule: a *hand-off* rc artefact is never the thing published to the appcast. Governs R21, R22.
  - **DEVIATED 2026-07-27 — the reused-string half was overridden; the hand-off build ships as `0.1.13-rc2` (build 17).** This is a deliberate decision, not drift. KTD7's reasoning has a hole that only surfaced when the build was ready to hand out: the first hand-off build had *already* gone to the reporter as `0.1.13-rc1` (build 15, the stage-B′ reordering that did not fix the crash), and the Steps 2–8 build was to be `0.1.13-rc1` again (build 16). Two different builds, same DMG file name, and — the load-bearing part — **the same thing on screen**: `NoType/UI/Settings/Components/VersionBlock.swift` renders `CFBundleShortVersionString` and nothing else, so the build integer never reaches the About surface. KTD7 made the rounds distinguishable *in the issue log* but not *to the tester holding the file*, and a hand-off round's entire value is attributable evidence (cf. KTD8, which spends an extra launch buying exactly that attribution). **What survives unchanged:** bump `CFBundleVersion` once per hand-off build (16 → 17) so the log carries a second identifier; and KD5 — a hand-off rc artefact is **never** published to `docs/appcast.xml`. R21 ("hand-off builds are distinguishable per round") is unchanged and now satisfied on the surface the tester actually reads. The operational half of this note is repeated in the hand-off checklist's §2 pre-flight, where the numbers are verified.
...
- **Approach:**
  ...
  3. **Version (R21).** ~~Keep `CFBundleShortVersionString` at `0.1.13-rc1`; bump `CFBundleVersion` from `15` for the hand-off build.~~ **Superseded by KTD7's deviation note (2026-07-27):** mint a *new* string per hand-off build — this one ships as `CFBundleShortVersionString = 0.1.13-rc2`, `CFBundleVersion = 17` — because About renders only the version string, so a reused string leaves the tester unable to tell the rounds apart. Bumping the integer per hand-off build still applies. Edit `NoType/Info.plist` with the Edit tool — `PlistBuddy` reorders keys and drops the file's XML comments.
...
- **Approach:**
  ...
  5. Record the outcome on issue #82 with their macOS build and the app build (R2), one entry per tester. A crash **with** an `OBJC THROW` line is a completed round with a negative result and the log names the next target (R3). A crash with **no** `OBJC THROW` line goes to the **amended** AE4 — check the R4a armed line first, then read it as *the interceptor missed a throw* (rethrow / non-ObjC throw / log rollover), **not** as the premise failing; do not add a fifth suspect and do not remove the reader at the crash site. A CoreML/Espresso name in the log re-opens U6 for a follow-up round.
...
- **Tester A's field crash report on `0.1.13-rc2`, 2026-07-30 — the second independent confirmation of the mechanism, on a build that already carried the fixes and the interceptor.** Build 17, macOS 26.2 (25C56). It carries the `HIServices SOME_OTHER_THREAD_SWALLOWED_AT_LEAST_ONE_EXCEPTION` marker thread, and the family's exact crash signature: SIGSEGV `KERN_INVALID_ADDRESS` at `0x2`, `swift_getObjectType` ← `swift_task_isMainExecutorImpl` ← `swift_task_isCurrentExecutorWithFlagsImpl`, faulting on a mouse-down through `NSGestureRecognizer` → SwiftUI `FullGestureCallbacks`. Timeline: launch `17:58:56.53` → marker thread `17:59:01.716` (T+5.2 s) → crash `17:59:03.04` (1.3 s after the swallow). Three inferences, in order of how much they carry:
  - **An ObjC exception was swallowed on a fixed build.** The interceptor shipped in `643ad17`; rc2 is `59ff2a7`, so it was present. This is what makes the Goal Capsule's *"armed and silent ⇒ nothing threw"* reading unsound.
  - **The preprocessor chain was intact when it happened.** The marker thread's name embeds a hash HIServices computes from `NSException.callStackReturnAddresses`; Step 2's review measured that a fresh `NSException` carries **zero** of those until Foundation's preprocessor populates them, and an empty array aborts HIToolbox at `HIExceptions.mm:45` (`SIGABRT`). No `SIGABRT` occurred, so Foundation's preprocessor ran.
  - **The interceptor is not displaced.** `NoTypeTests/ExceptionBreadcrumbDisplacementProbeTests.swift` force-loads every framework the app pulls in after launch (Sparkle, CoreML, AVFAudio, Vision, ScreenCaptureKit, AppKit, SwiftUI) plus the app host's full real launch sequence, and finds the preprocessor head still ours after each. It is sentinel-validated — it correctly reports a known decoy head rather than "still ours" — so a "still ours" result is a measurement, not blindness.

  What this does **not** establish: which of NoType's calls throws. That is still unproven, the ranked suspects are still suspects, and the crash is not fixed.
