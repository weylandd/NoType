...
- **Stop condition.** The reporter completes onboarding and operates the UI without a crash. That alone is the bar (KD4) — a surviving `OBJC THROW` line after a clean run does **not** hold the issue open; it is a follow-up, handled by AE3. A round that still crashes is a completed round with a negative result — record it and move on, do not retry it in variations.
...
- **Open blockers.** None blocking Steps 1–8. The two prior call-outs are settled (KD6, KTD1). Two release-shaped questions are open and are owned by the maintainer, not by an implementer — how long the reporter runs the build before closeout, and what happens if the reporter goes quiet. Both are in Outstanding Questions; neither blocks starting.
...
- AE2. The diagnostic comes back inconclusive
  ...
  - **Then** it admits **three** explanations and the plan must not pick one: an intermediate `@try` caught the throw (Sparkle, CoreML internals); `NSApplicationCrashOnExceptions` does not cover the HIToolbox/Carbon swallow path the `HIServices` breadcrumb names; or the default was never applied.
...
- AE4. The crash survives and the interceptor is silent — the falsifier
  - **Covers R3, R4a.**
  - **Given** a hand-off build carrying the interceptor and the Step 3–5 fixes, and the reporter still crashes,
...
**Open — raised by document review, and both are maintainer calls the plan must not make for itself:**

- **How long does the reporter run the hand-off build before the README note comes off and #82 closes?** The plan as written closes on one bounded session. That is the same evidentiary bar that pronounced each of the three prior call-site fixes successful, each time before the crash reappeared at the next site — and this crash is latent, arbitrary in where it lands, and device-set dependent. A minimum window of ordinary daily use is the obvious guard; the length is a product judgement about how long to leave a public known-issue note up. Step 9 item 5 carries the caution; it deliberately does not pick a number.
- **What happens if the reporter goes quiet?** KD5 keeps rc builds off the appcast and KTD7 gates promotion to `0.1.13` on their confirmation, so a silent reporter means the interceptor and the three latent-bug fixes reach nobody, indefinitely, while the Problem Frame says new users on affected machines are uninstalling silently today. The options are: wait; time-box it and ship on the latent-bug merits with the README note left in place; or recruit a second affected user first (already in Deferred). Trading "unverified fix reaches everyone" against "verified fix reaches nobody yet" is a release decision, not an implementation one — escalate rather than letting an implementer's default settle it.
...
- KTD8. **Drop the "ask the reporter to defer macOS updates" constraint, but replace it with a baseline arm.** The superseded plan froze them on 25C56 because the hypothesis was OS-shaped. The exception is ours and fires on any build, so recording the build stays required (R2) but freezing it does not. **However**, dropping the freeze *and* the superseded plan's baseline re-check together would leave the verification confounded: Apple is actively tracking the non-exception-safe `ExecutorTrackingInfo` pop, so if the reporter's macOS moves between Step 1 and Step 9, an OS-side change looks identical to this plan's fix — and that one result is the only evidence there is. So Step 9 runs **two arms in one session**: the reporter first re-launches their currently-installed pre-fix build and confirms it still crashes, then runs the hand-off build. If the pre-fix build no longer crashes, the round is **void as fix evidence** and is recorded as a suspected OS-side change. This costs one extra launch — they already have the old build installed. Governs R2, R17.
...
- **Approach:**
  ...
  1. Ask the reporter to run `defaults write app.notype NSApplicationCrashOnExceptions -bool YES` against their **already-installed** build, reproduce the crash, and attach the new `.ips`. The README (`## Known issues`) already carries this instruction verbatim — this step is the direct ask plus the recording. **Untested assumption, state it as one:** that this default intercepts the swallow the evidence actually names — HIToolbox/Carbon event dispatch, per the `HIServices` frame — rather than only exceptions reaching `NSApplication`'s own top-level handler. If it does not, this step returns a false negative on every candidate at once. It is testable locally against the reproduced probe; do that before reading an unchanged report as meaningful.
...
- **Branch on the result:**
  ...
  - **Reporter's macOS build has moved** → not a problem and not a re-run: per KTD8 the exception is ours and fires on any build, so record the new build (R2) and read the result normally. The one case that *does* change things is the crash no longer reproducing at all — then the plan has lost its only verifier and the Step 9 escalation below applies.
  - **Reporter unavailable, or goes quiet mid-flight** → Steps 2–5 ship anyway and Step 6 is recorded as skipped; verification parks. This leaves the plan with **no terminal state** — Step 9 cannot complete, the README note cannot come off, and the family entry's suspect list stays unresolved. There is no timeout in this plan because whether to promote an unverified build to a real release is a maintainer call, not an implementer's; see Deferred (recruiting a second affected user) and escalate rather than deciding it in-flight.
...
- **Approach:**
  ...
  2. **Run the round as two arms in one session (KTD8).** First have them re-launch their currently-installed **pre-fix** build and confirm it still crashes on their current macOS. Only then the hand-off build. Without this arm a "no crash" is not attributable — an OS-side change to the `ExecutorTrackingInfo` pop would look identical to this plan's fix, and this is the plan's only evidence. If the pre-fix build no longer crashes, the round is **void as fix evidence**; record it as a suspected OS-side change.
  3. Ask them to complete onboarding, exercise buttons across the main window and popover, and then run the README's `log show` command **whether or not it crashed**. Two things to tell them up front: (a) **if the mic-check spectrum meter stays flat, that is the new format guard declining an unsafe tap, not a broken microphone** — continue through onboarding and send the log either way; without this warning the most likely success case reads to them as a failure. (b) Read the log output before posting it (R7a).
  4. Record the outcome on issue #82 with their macOS build and the app build (R2). A crash **with** an `OBJC THROW` line is a completed round with a negative result and the log names the next target (R3). A crash with **no** `OBJC THROW` line is the falsifier — check the R4a armed line first, then follow AE4; do not add a fifth suspect. A CoreML/Espresso name in the log re-opens U6 for a follow-up round.
  5. **Attribution caution before closing.** One bounded session — complete onboarding, click some buttons — is the same evidentiary bar that declared each of the three prior call-site fixes successful, each time before the crash reappeared elsewhere. This crash is latent by construction, arbitrary in where it lands, and device-set dependent. How long the reporter should run the build before the README note comes off is a maintainer call, not an implementer's; do not close on the first clean session by default.
  6. **On confirmation (R18):** remove the `## Known issues` block from `README.md`, and **move** the `log show` retrieval command plus its R7a read-before-posting note into a short standing `## Reporting a crash` section — the interceptor is permanent and that command stays its only surface (KD6; do not substitute an in-app affordance on the way out). Naming the destination matters: Step 2 places the recipe *inside* the block this step deletes. Rewrite the family entry's "Suspected throwers — NOT confirmed" section into the named cause, **keeping the ranked list as history** so a future recurrence starts from evidence. While there, correct that entry's "roughly once per second by the permission poll" claim about the HUD reposition loop — it repeats the same error Step 4 corrects. Cross-link the closing commit. Close issue #82.
  7. **State the limit plainly in the closing note.** The maintainer's machine does not reproduce the crash. Local gates proved only that behaviour did not regress; the reporter's result is the only evidence that matters, and it is n=1.
- **Test expectation:** none — the verification is the reporter's report, captured by F1.
- **Done when:** issue #82 carries the verification outcome with both build strings and both arms of the KTD8 baseline, and exactly one of: the issue is closed with the README note removed and the family entry updated; the run was clean but the interceptor named a surviving thrower, in which case the issue still closes under KD4 and the thrower is folded into R14's audit (AE3); a crash was recorded with the interceptor naming the next target (R3); or a crash was recorded with a silent-but-armed interceptor, which is the premise-level negative of AE4.
...
- Issue #82 records whether its own attached `.ips` carries the `SOME_OTHER_THREAD_SWALLOWED_AT_LEAST_ONE_EXCEPTION` breadcrumb — the premise check that costs no reporter round-trip — alongside the zero-build diagnostic result, the reporter's macOS build, their onboarding step, and their audio device list.
- `ExceptionBreadcrumb.install()` is the first statement of `NoTypeApp.init()`, ships in release, logs at `.fault` behind a per-launch record cap, emits the R4a armed line and a distinct `dlsym`-failed line, scrubs `reason` through `SecureFieldMasker.scrubContent` before logging it, returns exceptions unchanged, chains to any preprocessor it replaced, and is pinned by a presence assertion that was observed red. Its only user-facing surface is the README recipe — no in-app diagnostics affordance ships (KD6).
