...
**Issue #82 was closed on 2026-08-13, and what that closure means is narrower than "fixed".** No report has arrived since 2026-07-30; the thrower was never named; nothing shipped in `0.1.13` or `0.1.14` addresses it beyond the three candidates already in rc2. It was closed as a maintainer call to stop carrying an issue nobody can act on without new evidence — **absence of reports, on a product with no telemetry, is not evidence of absence** (this entry's own *Why This Matters* is the case study for that). The README's known-issue note is deliberately **kept**, retuned to say exactly this and to name where a new report goes. Treat the closure as an inbox decision; treat this entry as the state of the diagnosis. One arriving report reopens it, and the suspect list below is still where the next reader starts.
...
What is guarded instead is only where a **named chokepoint makes the set genuinely closed** — `RaiseSiteScanner` in `NoTypeTests/HUDPanelGeometryTests.swift`, over exactly two files: every `setContentSize` / `setFrameOrigin` / `setFrame` in `HUDPanel.swift` must sit inside `applyValidated`; every `installTap` / `removeTap` in `MicProbe.swift` inside `installTapAndStart` / `removeTapIfInstalled`. Note the predicate is *positional*, not an absence needle — a guarded call still contains the literal `setContentSize(`, so "this string does not appear" cannot express the rule at all. It carries the presence complement (the mutator still occurs, the chokepoint is declared, something calls it, **and each body that performs the mutation still contains the validation** — checked per body, so a second un-validating overload is not excused by its validating sibling). Its limits are written down beside it, measured by trying to defeat it rather than reasoned about: `HUDController.swift`'s geometry calls are outside the closed set; the mutator list is an enumeration, so a geometry API absent from it (notably the `frame` property setter) passes silently; a *new kind* of mutator added inside an existing wrapper passes; and `mustValidate` is a substring test, so a predicate whose result is discarded satisfies it. The general case is covered by the interceptor above, not by a scan.
...
- **`MicProbe.installTapAndStart()` — `AVAudioEngine.installTap` format mismatch.** `installTap` raises `NSException` (`com.apple.coreaudio.avfaudio`, *"Input HW format and tap format not matching"*) whenever the tap format differs from the input node's live hardware format — reproduced on demand in a standalone probe. The device switch goes through `AudioUnitSetProperty(kAudioOutputUnitProperty_CurrentDevice)`, which is **asynchronous**, but the format is read immediately afterwards. It is not a Swift error, so the surrounding `do/catch` cannot catch it. `rebuild()` is invoked from two Swift-concurrency jobs (the config-change observer and the device-observation loop), which is exactly the §1 requirement. Device-set dependent — which would explain why the maintainer's machine is clean. Corroborating, not conclusive: the crash report's live threads include `com.apple.audio.toolbox.AUScheduledParameterRefresher`, which requires an instantiated AudioUnit, and `MicProbe` is the app's only `AVAudioEngine` user.
  **Its exposure changed in kind on 2026-08-13 (0.1.14), and the shape of the change matters more than its size.** Fixing [issue #86](https://github.com/weylandd/NoType/issues/86) moved the `AudioDeviceManager.apply` pin out of `start()` and into `installTapAndStart()`. The naive reading is "more installs now race a switch"; the load-bearing reading is that **`start()` was never in this suspect class to begin with**. It runs from `OnboardingMicCheckStep`'s `.onAppear` — a synchronous main-actor SwiftUI callback, not a concurrency job — so a raise there orphans no `ExecutorTrackingInfo` and fails §1 of the mechanism outright. While the pin lived only there, the *self-provoked* switch never overlapped a `Task`-borne install at all, and what `rebuild()` raced was an **externally** caused device change landing in the window by coincidence. After the fix, the self-provoked switch overlaps the two `Task`-borne paths directly.
  **What bounds it is the idempotence, not the gate.** `apply` read-backs the current device and sets only on a genuine difference, so the overlap is **one racing install per real user device change** — not per install, not per rebuild, and specifically *not* on the configuration-change rebuild that the switch itself provokes, where the device is already current. One racing install per switch is the irreducible cost of switching the microphone; the alternative was a picker that never switched it. Do not read the format gate as the bound: it narrows the window to a few instructions between its `live` read and `installTap`, which is a *probability* argument, and this entry's whole thesis is that probability arguments about swallowed exceptions are how two months went missing. If a reporter's `OBJC THROW` ever names `com.apple.coreaudio.avfaudio` on a build ≥ 0.1.14, that promotes this suspect and the fix shape is to make the install **wait** for the switch to land (compare the node's live format against `AudioDeviceManager.inputStreamFormat(for:)` on the target device, and let the config-change observer supply the retry) — never to move the pin back, which restores the silent-wrong-device bug.
- **`HUDPanel` NaN geometry reaching `setFrameOrigin` / `setContentSize`.** Measuring an `NSHostingView` before it has a stable window/screen context can yield a NaN `fittingSize`; `-[NSWindow setFrameOrigin:]` then raises `NSInvalidArgumentException` (*"Invalid parameter not satisfying: !((__x) != (__x))"*).
  **Event-driven, not periodic.** An earlier version of this bullet said *"driven roughly once per second by the permission poll's HUD reposition loop"*; that was wrong, and it was doing this suspect's ranking work, so it is corrected rather than deleted. `PermissionsViewModel.tick()` does rewrite its three `@Observable` properties every second, which wakes `AppState.observePermissions()` at ~1 Hz — but the loop gates the HUD side effect behind `if ax != lastAx || mic != lastMic` (`NoType/AppState.swift:551`), so an unchanged tick never reaches `reconcilePermissionHUD` → `HUDController.repositionPermissionPanels()` → `HUDPanel.positionTopRight`. The geometry mutation actually runs on: panel construction (`HUDPanel.init`, the one measurement taken with no stable window/screen context); an explicit `presentMissing` (launch, menu-bar click, hotkey press with the microphone missing); a **genuine** permission-status change; the observation loop's single entry-time reconcile; a card dismissal restacking the rest; and each non-permission HUD's own one-shot show.
  **What ranks this suspect is therefore *when* the measurement is taken, not how often.** `HUDPanel.init` measures `fittingSize` at exactly the moment the hosting view has no window and no screen — the NaN condition — so one panel construction is enough. Repetition was never the argument, and a corrected reading does not demote the suspect; it removes a false reason for promoting it.
  **And it is *mostly* — not entirely — dead for a user parked mid-onboarding.** Two of the three `presentMissing` callers are gated on `onboarding.isComplete`: the launch auto-show (`AppState.swift:454`) and `handleMenuBarOpened` (`:573`). The third is **not**. `handleHotkeyPress` calls `presentMissing([.microphone])` (`AppState.swift:870`) on any hotkey press while the microphone is ungranted, and the tap is already installed by then — Accessibility is granted at the wizard's permissions step, and the onboarding short-circuit (`onboardingHotkeyPressObserver`) is armed only on the *hotkey-check* step, so it does not cover the permissions or mic-check steps. A user sitting on the mic-check step with Accessibility granted and the microphone not yet granted therefore builds a `HUDPanel` on one stray Right Option press — on the very step where `MicProbe` is running.
  So the two suspects are **not** mutually exclusive, and this list must not be read as if they were. What is true is weaker and still useful: post-onboarding, this path is the live one and `MicProbe` is reachable only by re-opening the wizard (Settings → Reset Onboarding); mid-onboarding, `MicProbe` is live and this path needs that specific stray press. Treat a reporter's onboarding step as **evidence about which suspect is more likely, never as an exclusion of the other.**
