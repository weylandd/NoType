...
- **When you see `swift_task_isCurrentExecutorWithFlagsImpl` anywhere in a crash stack on macOS 26, split on the signal before you form a hypothesis.**
  - **`EXC_BREAKPOINT` / SIGTRAP, terminating in `_dispatch_assert_queue_fail`, on a non-main thread** — this entry's shape. Treat it as an actor-isolation mismatch first, not as a memory bug or a framework bug: look for a closure literal inside a `@MainActor` function that crosses a thread boundary at runtime. The runtime is telling you the truth.
  - **`EXC_BAD_ACCESS` / SIGSEGV at a small integer address, terminating in `swift_getObjectType` or `objc_opt_class`** — a different bug with a different cause. The runtime is reading a dead stack slot left behind when a swallowed ObjC exception unwound through `libswift_Concurrency`. Do not hunt for an isolation mismatch; read the *other* threads and look for `SOME_OTHER_THREAD_SWALLOWED_AT_LEAST_ONE_EXCEPTION`. See [`macos-26-executor-identity-check-family-2026-07-25.md`](./macos-26-executor-identity-check-family-2026-07-25.md).

  (The undifferentiated form of this rule — "treat it as an actor-isolation mismatch first", full stop — was propagated from here into the `.onHover` entry in 2026-05 and sent that investigation down the wrong path for two months. The split above is the amendment, 2026-07-26.)
...
- [`solutions/runtime-errors/macos-26-executor-identity-check-family-2026-07-25.md`](./macos-26-executor-identity-check-family-2026-07-25.md) — **this entry is that family's counter-example, not a member of it**, and the family's now-proven cause makes the separation stronger, not weaker. That family is a swallowed ObjC exception orphaning the *main thread's* `ExecutorTrackingInfo`, read later by an arbitrary innocent frame. This crash cannot be that: (1) `ExecutorTrackingInfo` is a per-thread stack node and this fires on `com.apple.audio.IOThread.client`, not the main thread; (2) it terminates in `dispatch_assert_queue` / `_dispatch_assert_queue_fail`, a libdispatch queue-identity test that never reads the Swift executor identity slot, so a poisoned slot could not produce it; (3) it was deterministic — first IOProc invocation, every recording attempt — while corruption faults are latent and delayed; and (4) its crash report carries no `SOME_OTHER_THREAD_SWALLOWED_AT_LEAST_ONE_EXCEPTION` breadcrumb. The runtime was **right** here: the closure genuinely was running off-main. The `@Sendable` fix was correct and final and has not recurred. Keep the two diagnoses apart; conflating them corrupts the family's evidence.
- [`solutions/runtime-errors/timelineview-mainactor-instance-method-crash-2026-05-16.md`](./timelineview-mainactor-instance-method-crash-2026-05-16.md) — **same runtime *frame*, different mechanism** (this bullet originally said "same underlying runtime mechanism"; that was wrong). Different framework (SwiftUI TimelineView vs Core Audio HAL), different fix (restructure the closure body vs add `@Sendable`). They cross-reference each other because both surface as "macOS 26 trap originating in `_swift_task_*Impl`" — start at whichever doc is closer to your crashing module, then use the signal split under *Prevention* to tell which of the two you actually have.
- [`solutions/runtime-errors/onhover-mainactor-inheritance-crash-2026-05-19.md`](./onhover-mainactor-inheritance-crash-2026-05-19.md) — **a member of the executor-identity family, which this entry is not** (originally described here as "third member", with this crash counted as the second — both numbers were wrong). SwiftUI `.onHover` modifier closures inheriting `@MainActor`; mitigated via the `dsOnHover` wrapper at `NoType/UI/DSComponents.swift` that pairs `@Sendable` (this file's pattern) with `Task { @MainActor in ... }` to bridge back for `@State` writes.
