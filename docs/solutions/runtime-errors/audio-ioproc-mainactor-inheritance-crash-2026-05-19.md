...
- **The opposite is also true:** if the closure body *does* need to touch `@MainActor` state, do not strip the isolation — instead, bridge explicitly with `MainActor.assumeIsolated { ... }` or hop via `Task { @MainActor in ... }`. Don't make `@Sendable` the reflexive answer; make it the answer when the body is honestly non-isolated.
