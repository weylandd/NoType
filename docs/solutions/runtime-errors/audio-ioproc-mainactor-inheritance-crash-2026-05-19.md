...
- [`solutions/runtime-errors/macos-26-executor-identity-check-family-2026-07-25.md`](./macos-26-executor-identity-check-family-2026-07-25.md) — **this entry is that family's counter-example, not a member of it.** The two docs cross-linked below plus a third incident (a stock SwiftUI `Button`, 2026-07-25) share one signature — SIGSEGV at a small integer address in `swift_getObjectType`, on the main thread — and were reframed as one poisoned main-executor identity whose cause is still open. This crash only shares the `swift_task_isCurrentExecutorWithFlagsImpl` frame: it is `EXC_BREAKPOINT` on the HAL thread, terminating in `_dispatch_assert_queue_fail`, and the runtime was **right** — the closure genuinely was running off-main. The `@Sendable` fix here was correct and final and has not recurred. Keep the two diagnoses apart; conflating them corrupts the family's evidence.
