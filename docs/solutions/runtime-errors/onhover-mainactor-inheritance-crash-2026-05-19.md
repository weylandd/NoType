...
> **Annotation (2026-07-26) — this fix is a mitigation, not a cure.** The cause of this crash family has since been proven, and it is not `.onHover`. A swallowed Objective-C exception, thrown on the main thread inside a Swift-concurrency job, unwinds through `libswift_Concurrency` and orphans the thread's `ExecutorTrackingInfo`; AppKit resumes execution, and the **next** code to perform an executor check faults on the dead stack slot. The `.onHover` closure prologue below was one such reader — `dsOnHover` stopped this site from crashing and the crash reappeared two months later inside Apple's own `_ButtonGesture`, where no app closure exists to annotate. `dsOnHover` stays mandatory (it is good practice and it did remove a reader) but it does not close the class.
>
> **The answer was in this incident's own crash report the whole time.** That `.ips` carries a parked thread — thread 13 — whose stack contains `SOME_OTHER_THREAD_SWALLOWED_AT_LEAST_ONE_EXCEPTION`, timestamped ~675 ms before the SIGSEGV. Nobody read past thread 0. Read [`macos-26-executor-identity-check-family-2026-07-25.md`](./macos-26-executor-identity-check-family-2026-07-25.md) first. Everything below is preserved as the historical record.
...
> **Superseded numbering (2026-07-25).** The enumeration below is the diagnosis as it stood in 2026-05. It has since been reframed: entry 2 (Core Audio IOProc) is the family's **counter-example**, not a member, so this entry is the *second* same-signature incident, not the third — and a third one followed at a stock SwiftUI `Button`. The list is kept verbatim as the historical record; read [`macos-26-executor-identity-check-family-2026-07-25.md`](./macos-26-executor-identity-check-family-2026-07-25.md) for the current framing.
...
> **Corrected (2026-07-26).** SwiftUI does not "set up an invalid SerialExecutorRef". The identity slot already held garbage before `.onHover` ran: an ObjC exception thrown ~675 ms earlier, inside a Swift-concurrency job on the main thread, unwound through `libswift_Concurrency` and orphaned the thread's `ExecutorTrackingInfo` — a stack-allocated thread-local node whose pop is not exception-safe. `0x1` is whatever later reused that stack memory. `HoverResponder.updatePhase(_:)` is simply the caller that asked next. This does not change what `dsOnHover` does, but it does change what it *achieves*: it removes a reader of corrupt state, not a source of it.
...
- **When you see `swift_task_isCurrentExecutorWithFlagsImpl` anywhere in a crash stack on macOS 26.x, treat it as an actor-isolation mismatch first** (per the rule in [`audio-ioproc-mainactor-inheritance-crash-2026-05-19.md`](./audio-ioproc-mainactor-inheritance-crash-2026-05-19.md)) — even when the failing thread is the main thread. The hover crash is the counter-example to "main thread = safe" — the executor identity check faults regardless of which thread the closure runs on.
  > **Amended 2026-07-26.** Keep this only for `EXC_BREAKPOINT` / `_dispatch_assert_queue_fail` shapes, where the runtime is genuinely reporting a mismatch. For a **SIGSEGV at a small integer address** — this entry's shape — an isolation mismatch is the wrong first hypothesis: read the *other* threads and look for `SOME_OTHER_THREAD_SWALLOWED_AT_LEAST_ONE_EXCEPTION`.
- **Be suspicious of any SwiftUI modifier whose closure is invoked through SwiftUI's runtime dispatch machinery on macOS 26.x.** `TimelineView` (PR #41), `.onHover` (this entry), `.onContinuousHover`, `.onMove`, `.onDrop`, and the new HoverResponder/PressResponder paths all expose the same risk. The dispatch path doesn't have to cross a thread boundary to trip the executor identity check — it only has to hand the runtime a malformed executor reference.
  > **Amended 2026-07-26.** The risk is real but the model is backwards: none of these paths hands the runtime a malformed reference. They are all *readers* of a main-executor identity that some earlier swallowed exception poisoned. Any of them can be the crash site; none of them can be the cause. Suspicion is better spent on code that can raise an `NSException` inside a `Task { @MainActor }`.
...
- [`macos-26-executor-identity-check-family-2026-07-25.md`](./macos-26-executor-identity-check-family-2026-07-25.md) — **read this first.** This entry is the *second* of three same-signature incidents, now traced to one proven cause: a swallowed ObjC exception orphaning the main thread's `ExecutorTrackingInfo`. Three corrections to the framing below: the `dsOnHover` fix is mitigation at one call site, not coverage of the class (a stock SwiftUI `Button` crashed the same way two months later, inside Apple's own `_ButtonGesture`, where no NoType closure exists to annotate); the audio-IOProc crash listed below as "second instance" is the family's **counter-example**, not a member — different signal, thread, and terminal frame, and a genuine isolation violation; and the invalid executor identity is not something SwiftUI produced.
...
- [`audio-ioproc-mainactor-inheritance-crash-2026-05-19.md`](./audio-ioproc-mainactor-inheritance-crash-2026-05-19.md) — **not an instance of this family** (this bullet originally called it "second instance … same underlying mechanism"; that was wrong). It shares only the `swift_task_isCurrentExecutorWithFlagsImpl` frame: `EXC_BREAKPOINT` on the Core Audio HAL thread, deterministic, no swallowed-exception breadcrumb, and a genuine isolation violation. This entry's fix borrows its `@Sendable` shape plus a MainActor bridge to keep `@State` writes compiling — the shared shape is a coincidence of Swift syntax, not of cause.
