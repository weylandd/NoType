...
- [`runtime-errors/audio-ioproc-mainactor-inheritance-crash-2026-05-19.md`](audio-ioproc-mainactor-inheritance-crash-2026-05-19.md) — second instance of the macOS 26 executor-check family. Different framework (Core Audio HAL), same underlying mechanism, different fix shape (`@Sendable` strips inheritance from a non-Swift framework callback).
- [`runtime-errors/onhover-mainactor-inheritance-crash-2026-05-19.md`](onhover-mainactor-inheritance-crash-2026-05-19.md) — third instance. SwiftUI `.onHover` modifier closures inheriting `@MainActor`. Fixed via the `dsOnHover` wrapper in `NoType/UI/DSComponents.swift` (`@Sendable` strips inheritance, `Task { @MainActor in ... }` re-enters MainActor for `@State` writes).
