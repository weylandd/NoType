...
severity: high
applies_when:
  - Choosing the input device for AVAudioEngine
...
When NoType calls `AVAudioEngine.start()` against the default input, and that default is the BT headset, macOS:
...
End result: music plays louder *and* worse for ~1 second after the user presses the hotkey. The hypothesis was confirmed by the (now obvious in hindsight) fact that the bug only repros with Bluetooth output devices that also expose an input stream.
...
Default for `preferBuiltInOverBluetooth` is **ON**. The opt-out lives in Settings → "Prefer built-in mic with Bluetooth headphones" so power users who genuinely want BT input can disable it. Mac mini / Mac Studio users without a built-in mic gracefully degrade — step 2's "a built-in device exists" guard fails and step 3 returns the system default unchanged.
...
- `NoType/Recording/AudioRecorder.swift` — per-session log of the fallback firing.
- `NoType/UI/SettingsView.swift` — the opt-out toggle.
