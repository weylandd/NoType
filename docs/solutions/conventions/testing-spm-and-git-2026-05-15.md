...
- [a fixture must carry information the production path cannot regenerate](./fixture-must-carry-what-production-cannot-regenerate-2026-08-13.md) — the third member of the fixture family above, and the one the probe cannot catch: the probe is a question, and a fixture that cannot express the failure answers "green" to every question.
- [test-host side effects escape the test process](./test-host-side-effects-escape-the-test-process-2026-08-13.md) — what a test does to the machine rather than to its assertions; where to place the suppression so the coverage survives it.
