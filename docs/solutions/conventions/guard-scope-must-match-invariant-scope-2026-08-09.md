...
- [`reconcile-optimistic-mirror-by-union`](reconcile-optimistic-mirror-by-union-2026-08-09.md) — the U5 sibling. This entry asks *where the check goes*; that one asks *what set the check is keyed on*. Both are a check written at a narrower extent than the state it governs, and both fail only when two events line up.
- [`gate-irreversible-actions-on-the-outcome`](gate-irreversible-actions-on-the-outcome-2026-08-09.md) — the U6 sibling, completing the triad: *where* the check goes (this entry), *what set* it is keyed on (U5), *which proposition* it actually asserts (U6). That one is not a scope mismatch — its predicate and its action sit at the same altitude — so the tell above will not find it.
- [`cited-invariant-must-cover-the-population`](cited-invariant-must-cover-the-population-2026-08-11.md) — the citation-level cousin. This entry asks where a *check* goes; that one asks whether the *invariant a comment cites* covers the population the code actually touches. No check is misplaced there and no code moves, so the tell above will not find it — the mis-scoped term is in prose.
