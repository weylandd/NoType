...
> **The convention holds; the machinery it is narrated over does not (2026-08-13).** Everything below describes the code as it stood on 2026-08-09, when a history row *was* the pasted string and the merge found its slot by scanning that string for the *i*-th `[…]`. `docs/plans/2026-08-11-001-fix-dictation-delivery-reliability-plan.md` replaced that model: a row stores an ordered response sequence, a gap is a **position** rather than a character, and `RetryMerge.merge(into:outcomes:)` writes each recovery into the gap covering that chunk's own index. So the specific defect narrated here — a dictionary pair rewriting the markers out from under the merge — is now structurally unreachable, `mergeDetailed` and `merge(existingText:recovered:)` no longer exist, and nothing decrements `failedChunkCount` (it is derived from the sequence).
>
> **Read the "Right" block below as history, not as the current shape.** What survives, and survives *because* it was written as a postcondition rather than as marker arithmetic: `Merged.placed` is still the flag every release reads, and `settleRetry` still frees a chunk's audio only for outcomes that actually landed. Writing by index made the mismatch rare — a recovery aimed at an index no gap covers, e.g. a second run against a row a first already recovered — and rare is exactly the condition under which this entry says a proxy is most dangerous. The one-line version of why this entry outlived its own bug: a guard keyed on *the operation reported what it achieved* needed no edit when the operation was rewritten, and a guard keyed on *what came back off the wire* would have needed one.
...
**Right** (`e1101e6`) — the merge records placement where it happens, and all three decisions read it. *(The shape below is the 2026-08-09 marker-scanning merge, retired 2026-08-13 — see the note under the title. What it demonstrates, and what survived the rewrite, is the `placed` flag being set at the point of placement rather than inferred by the caller.)*
...
- `NoType/History/RetryMerge.swift` — the enum doc-comment carries the current mechanism (the index write, and why `placed` still exists separately from `isRecovery`) **and** the retirement of the scan narrated here, under "That is a correction, not a refinement". The symbols this entry names — `mergeDetailed`, `merge(existingText:recovered:)`, `recoveredCount` — are gone; the live ones are `merge(into:outcomes:)`, `Merged.placed`, `isRecovery`, `isEmptyText`, `priors(from:)`.
- `NoType/AppState.swift` — `settleRetry` and its three exits, all still keyed on `placedCount`; the destructive `take` that makes every exit load-bearing. `NoType/History/RetainedAudioStore.swift` — the "only copy" contract this entry rests on. (Line numbers from 2026-08-09 have all moved; search by symbol.)
- `NoType/Dictionary/TextReplacementEngine.swift` — the Unicode look-around that made the mismatch reachable. It still matches the `…` inside `[…]`, and that is now harmless-by-construction: replacement runs at *render* time, downstream of assembly, so a pair restyles a gap instead of deleting one. `NoType/Dictionary/CLAUDE.md` carries the before/after.
...
- [`partial-recovery-with-markers`](../architecture-patterns/partial-recovery-with-markers-2026-05-16.md) — where the `[…]` marker comes from, why markers are rewritable, and (since 2026-08-13) why the gap is stored as a position instead. The retry feature and the retention contract themselves are owned by the module docs, not here.
