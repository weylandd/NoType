...
- [A mic-picker change during a recording session does not reach `AudioRecorder`](solutions/documentation-gaps/picker-device-change-not-observed-mid-session-2026-08-13.md) — **S**; the session's rebuild listens for the *system* default changing, not for the app's own picker. Same shape as issue #86, deliberately not fixed with it: nothing on screen claims the switch happened, so nobody is misled.
