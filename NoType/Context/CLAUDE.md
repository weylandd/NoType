...
6. **`InsertionTarget` is captured once at session start, and the prompt ships that capture unchanged.** The section ships even when `.unknown` — empty strings, but the section stays so the prefix shape is stable.

   **The justification for capturing once is no longer "the cursor doesn't move because the user is holding the hotkey."** That was true of push-to-talk and is false of the hands-free lock: a locked session lets the user walk to another application and tap to stop there, and since the destination guard the paste follows them — the transcript goes to the process they *stopped* in (`NoType/Recording/CLAUDE.md` "Destination guard"). What survives is narrower and still sufficient: `Insertion target:` is one of the cache-prefix parts that must stay **byte-stable across the chunks of one session** (`NoType/Gemini/CLAUDE.md` invariant 3), so re-reading the cursor mid-session would cost every subsequent chunk its cache hit.

   The consequence that used to be covered by the false premise is handled at the *paste*, not here: `RecordingSession` consults the pure `shouldDiscardInsertionContext(sourcePID:destinationPID:)` and hands `TextInjector.finalizeForInsertion` an `.unknown` target when the session started in one process and is pasting into another — because `finalizeForInsertion` does not merely guess there, it strips a sentence-final `.` / `!` / `?` the user dictated whenever the other application's `textAfter` looks mid-sentence. So a stale capture still reaches Gemini (harmless — it is context, and it describes where the dictation began) and is deliberately withheld from the insertion normalisation (not harmless — it edits the user's words).
