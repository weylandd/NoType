...
- All three are driven by closures on `NoTypeAppDelegate`, assigned in `NoTypeApp.init()` (assigning a closure schedules nothing and touches no `NSApp`) and invoked from AppKit's launch callbacks. Two hooks, deliberately:
  ...
  - `launchHandler` → `applicationDidFinishLaunching(_:)` → `appearance.apply()` again (idempotent fallback), then `state.prime()`. Appearance stays **before** priming so the theme is on `NSApp` ahead of any UI priming can surface.
- **Don't move this to a scene's `.task`.** NoType is `LSUIElement` and the main window isn't presented at launch once onboarding is complete, so a returning user would never fire it. `wireTerminationHandler()` and `updates.start()` are still on that hook — they are the counter-example, not the precedent, and remain a known gap.
