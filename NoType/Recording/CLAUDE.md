...
- The outcome rides `SessionSummary.pasteWithheldForDestinationChange`, and the stop-moment application's name rides `SessionSummary.pasteDestinationAppName` beside it (KTD7) — frozen so the notice can name where the transcript was headed without re-reading the frontmost app at notice time (R25). The user-facing *notice* that consumes both is a separate unit; until it lands, a withheld session is silent apart from a `.notice` log line.
