...
**Where this stands.** We closed the internal errors we could find, and added a watcher that names one if it still fires. We have had no report of the crash since — but that is *absence of reports*, not proof it is gone, and NoType has no telemetry, so those are the same thing from where we sit until someone tells us. The tracking issue ([#82](https://github.com/weylandd/NoType/issues/82)) is closed on that basis and will be reopened the moment a report arrives. If this is happening to you on a current build, please say so — you are the only signal there is.
